...
public class InWorldUIElement : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        playerTransform = canvas.transform.parent.Find("Player Head Position");
        mainCamera = playerTransform.Find("Camera").GetComponent<Camera>();
    ...
    private void Update()
    {
        ...
        {
            float distance = Vector3.Distance(playerTransform.position, objectToFollow.position);
            float childrenScale = 1f - distance.Remapped(minDistance, maxDistance, 0f, 1f).CustomSmoothstep();
